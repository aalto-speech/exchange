...
// Test removing the last word from a class
void exchangetest::ExchangeTest6(void)
{
    cerr << endl;
    Exchange e(2, "test/corpus1.txt");

    int widx = e.m_vocabulary_lookup["a"];
    int curr_class = e.m_word_classes[widx];
    int new_class = (curr_class == 3) ? 2 : 3;

    e.do_exchange(widx, curr_class, new_class);

    widx = e.m_vocabulary_lookup["c"];
    curr_class = e.m_word_classes[widx];
    new_class = (curr_class == 3) ? 2 : 3;

    e.do_exchange(widx, curr_class, new_class);

    double test_ll = e.log_likelihood();
    ...
    widx = e_ref.m_vocabulary_lookup["a"];
    curr_class = e_ref.m_word_classes[widx];
    new_class = (curr_class == 3) ? 2 : 3;
    ...
    CPPUNIT_ASSERT( ref_ll == test_ll );
    CPPUNIT_ASSERT( e.m_classes[curr_class].size() == 0 );
    CPPUNIT_ASSERT( e_ref.m_classes[curr_class].size() == 0 );
